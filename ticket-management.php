...
if (isset($_POST['btn_confirm_delete'])) {
    ...
    $sql = "SELECT status FROM tblticket WHERE ticketnumber = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "s", $ticketToDelete);
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if ($logStmt = mysqli_prepare($link, $sql)) {
                ...
                if (mysqli_stmt_execute($logStmt)) {
                    ...
                    $errorMessages[] = "Could not log the delete action.";
                }
                mysqli_stmt_close($logStmt);
            }
        } else {
            $errorMessages[] = "Could not delete ticket.";
        }
        mysqli_stmt_close($stmt);
    }
}

if (isset($_POST['btn_confirm_approve'])) {
    $ticketToApprove = trim($_POST['approve_ticketnumber']);
    $sql = "SELECT status FROM tblticket WHERE ticketnumber = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "s", $ticketToApprove);
        if (mysqli_stmt_execute($stmt)) {
            $result = mysqli_stmt_get_result($stmt);
            $ticket = mysqli_fetch_assoc($result);
            if ($ticket && $ticket['status'] === 'WAITING FOR APPROVAL') {
                ...
                $errorMessages[] = "Only tickets with status 'WAITING FOR APPROVAL' can be approved.";
            }
        }
        mysqli_stmt_close($stmt);
    }
...
if (isset($_POST['btn_approve_yes'])) {
    $ticketnumber = trim($_POST['approve_ticketnumber']);
    $sql = "UPDATE tblticket SET status = 'CLOSED', approvedby = ?, dateapproved = ?, last_updated = ? WHERE ticketnumber = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        $approvedby = $_SESSION['username'];
        $dateapproved = date("Y-m-d H:i:s");
        $last_updated = date("Y-m-d H:i:s");
        mysqli_stmt_bind_param($stmt, "ssss", $approvedby, $dateapproved, $last_updated, $ticketnumber);
        if (mysqli_stmt_execute($stmt)) {
            $sql = "INSERT INTO tbllogs (datelog, timelog, action, module, performedto, performedby) VALUES (?, ?, ?, ?, ?, ?)";
            if ($logStmt = mysqli_prepare($link, $sql)) {
                $date = date("Y-m-d");
            ...
            $errorMessages[] = "Could not approve ticket.";
        }
        mysqli_stmt_close($stmt);
    }
}
...
if (isset($_POST['btn_confirm_assign'])) {
    $ticketnumber = trim($_POST['assign_ticketnumber']);
    $sql = "SELECT status FROM tblticket WHERE ticketnumber = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "s", $ticketnumber);
        if (mysqli_stmt_execute($stmt)) {
            ...
            if ($ticket && ($ticket['status'] === 'PENDING' || $ticket['status'] === 'ON-GOING')) {
                header("location: assign.php?ticketnumber=" . urlencode($ticketnumber));
                exit();
            } else {
                $errorMessages[] = "Only tickets with status 'PENDING' or 'ON-GOING' can be assigned.";
                $showAssignErrorModal = true;
            }
        }
        mysqli_stmt_close($stmt);
    }
    ...
    <div class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
            ...
            <section class="table-section">
                <?php
                $sql = "SELECT ticketnumber, problem, datecreated, time, status FROM tblticket ORDER BY datecreated DESC";
                if (isset($_POST['btnsearch'])) {
                    $search = '%' . trim($_POST['txtsearch']) . '%';
                    $sql = "SELECT ticketnumber, problem, datecreated, time, status FROM tblticket 
                            WHERE ticketnumber LIKE ? OR problem LIKE ? OR status LIKE ? 
                ...
                function buildTable($result) {
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "</form>";
                            echo "<form action='' method='POST' class='inline-form'>";
                            echo "<input type='hidden' name='assign_ticketnumber' value='$ticketnumber'>";
                            echo "<button type='submit' name='btn_confirm_assign' class='btn assign-btn'>";
                            ...
                            echo "<div class='action-row'>";
                            echo "<form action='' method='POST' class='inline-form'>";
                            echo "<input type='hidden' name='approve_ticketnumber' value='$ticketnumber'>";
                            echo "<button type='submit' name='btn_confirm_approve' class='btn approve-btn'>";
        ...
        <div id="approveConfirmModal" class="modal" style="<?php echo $showApproveConfirmModal ? 'display: block;' : 'display: none;'; ?>">
            <div class="modal-content complete-confirm-modal">
                <div class="modal-header">
                ...
                <hr>
                <p>Do you want to approve this ticket?</p>
                <hr>
                <form action="" method="POST" class="modal-form">
                    <input type="hidden" name="approve_ticketnumber" value="<?php echo htmlspecialchars($ticketToApprove); ?>">
                    <button type="submit" name="btn_approve_yes" class="btn modal-btn-complete"><i class="fa-solid fa-thumbs-up me-2"></i> Approve</button>
                    <button type="button" id="noApproveButton" class="btn modal-btn-cancel">Cancel</button>
                </form>
            </div>
        </div>
        ...
        <div id="assignErrorModal" class="modal" style="<?php echo $showAssignErrorModal ? 'display: block;' : 'display: none;'; ?>">
            <div class="modal-content error-modal">
                ...
                <hr>
                <p>Only tickets with status 'PENDING' or 'ON-GOING' can be assigned.</p>
                <hr>
                <button id="closeAssignErrorButton" class="btn modal-btn-close">OK</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.getElementById('noApproveButton')?.addEventListener('click', function() {
    ...
    document.getElementById('closeAssignErrorButton')?.addEventListener('click', function() {
        document.getElementById('assignErrorModal').style.display = 'none';
    });
</script>
</body>
</html>