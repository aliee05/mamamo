        ...
        if(mysqli_stmt_execute($stmt)) {
            ...
            if(mysqli_num_rows($result) > 0) {
                $accounts = mysqli_fetch_array($result, MYSQLI_ASSOC);
                               if(strtoupper($accounts['usertype']) === "USER") {
                    session_start();
                    $_SESSION['username'] = $accounts['username'];
                    $_SESSION['usertype'] = $accounts['usertype'];
                    header("location: user-dashboard.php");
                } else {
                    $error_message = "<span class='error-message'><i class='fa-solid fa-exclamation-circle'></i> This login is for USER accounts only.</span>";
                }
            } else {
    ...
    <div class="login-container">
        <div class="left-panel">
            ...
            <div class="system-info">
                <span class="status">USER <i class="fa-solid fa-circle-check"></i></span>
            </div>
        </div>
