...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errorMessages)) {
        ...
        if ($link === false) {
            $errorMessages[] = "ERROR: Could not connect to database. " . mysqli_connect_error();
        } else {
            $sql = "INSERT INTO tblticket (ticketnumber, problem, details, status, time, createdby, datecreated, assignedto, dateassigned, datecompleted, approvedby, dateapproved, last_updated) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            if ($stmt = mysqli_prepare($link, $sql)) {
                mysqli_stmt_bind_param($stmt, "sssssssssssss", $ticketnumber, $problem, $details, $status, $time, $createdby, $datecreated, $assignedto, $dateassigned, $datecompleted, $approvedby, $dateapproved, $last_updated);
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    if ($logStmt = mysqli_prepare($link, $sql_log)) {
                        ...
                        mysqli_stmt_bind_param($logStmt, "ssssss", $date, $time_log, $action, $module, $performedto, $_SESSION['username']);
                        if (!mysqli_stmt_execute($logStmt)) {
                            $errorMessages[] = "ERROR: Could not log the action to tbllogs. " . mysqli_stmt_error($logStmt);
                        }
                    ...
                    $errorMessages[] = "ERROR: Could not create ticket. " . mysqli_stmt_error($stmt);
                }
